...
// src/Deferred.ts
var Deferred = ({ children, data, rescue, fallback }) => {
  if (!data) {
  ...
  const rescuedKeys = useMemo2(() => new Set(page.rescuedProps), [page.rescuedProps]);
  useEffect2(() => {
    const removeStartListener = router3.on("start", (e) => {
      const visit = e.detail.visit;
      if (visit.preserveState === true && isSameUrlWithoutQueryOrHash(visit.url, window.location) && partialReloadRequestsSomeProps(visit, keys)) {
  ...
  const hasRescuedProps = useMemo2(() => keys.some((key) => rescuedKeys.has(key)), [keys, rescuedKeys]);
...
import { useCallback as useCallback2, useMemo as useMemo3, useRef as useRef3 } from "react";
...
import React2, { use as use3, useEffect as useEffect6, useMemo as useMemo5 } from "react";
...
var InfiniteScroll = forwardRef2(
  ({
    ...
    const [hasPreviousPage, setHasPreviousPage] = useState6(!!scrollProp?.previousPage);
    const [hasNextPage, setHasNextPage] = useState6(!!scrollProp?.nextPage);
...
import { createElement as createElement5, forwardRef as forwardRef3, useEffect as useEffect8, useMemo as useMemo7, useRef as useRef6, useState as useState7 } from "react";
var noop2 = () => void 0;
...
import { useCallback as useCallback4, useMemo as useMemo8, useRef as useRef7, useState as useState8 } from "react";
...
function usePoll(interval, requestOptions = {}, options = {
  ...
  const latest = useRef8(requestOptions);
  latest.current = requestOptions;
  const pollRef = useRef8(null);
  useEffect9(() => {
...
function usePrefetch(options = {}) {
  ...
  const [lastUpdatedAt, setLastUpdatedAt] = useState9(cached?.staleTimestamp || null);
  const [isPrefetching, setIsPrefetching] = useState9(inFlight !== null);
  const [isPrefetched, setIsPrefetched] = useState9(cached !== null);
  useEffect10(() => {
...
import { createElement as createElement6, useCallback as useCallback5, useEffect as useEffect11, useMemo as useMemo9, useRef as useRef9, useState as useState10 } from "react";
var WhenVisible = ({ children, data, params, buffer, as, always, fallback }) => {
  ...
  const [loaded, setLoaded] = useState10(() => keys.length > 0 && keys.every((key) => get3(pageProps, key) !== void 0));
